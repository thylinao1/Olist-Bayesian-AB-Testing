...
The point of this module is to make it easy to say:
    ...
    "On the marginal data, the two-proportion z-test gives p=0.04 and the
     ...
     even though the pooled comparison is null, which is Simpson's-paradox-
     style heterogeneity that a flat A/B analysis cannot see."

That is the headline result of the project. These are the tests a
marketplace data scientist would run by default, so the Bayesian result
has to be reported alongside them.

References
----------
- WAIC / PSIS-LOO for Bayesian model comparison (Vehtari et al.).
- Two-proportion z, Welch's t, Mann-Whitney U: standard introductory
  statistics, included here to show where they fall short on
  hierarchical data.
"""

...
### A portfolio project applying *Statistical Rethinking* (McElreath, 2nd ed.) to the Olist Brazilian E-commerce dataset
...
The DAG is encoded in [`src/dag.py`](../src/dag.py) and rendered to [`reports/figures/dag.png`](figures/dag.png). The four-elemental-confounds framework (fork, pipe, collider, descendant — McElreath §6.4) gives the **adjustment set `{C, S, G, M}`**: product category, seller volume tier, customer state, calendar month. Variables downstream of T — freight charged, basket size, delivery days, whether a review was left — are deliberately *not* conditioned on, because conditioning on a mediator is exactly the post-treatment bias §6.2 warns about. Conditioning on the review (a collider with both T and Y as parents) would open a non-causal path.
...
Each model uses the same adjustment set, the same hierarchical pattern, and the same non-centered parameterisation McElreath §13.4 recommends for stability.

### 4.1 Hierarchical Binomial — on-time delivery (Ch 11 + 13)
...
### 4.2 Hurdle-LogNormal — repeat-purchase revenue (Ch 12.2)

A two-stage mixture identical in spirit to McElreath's drinking-monks ZIPoisson, but adapted for continuous revenue:
...
### 4.3 Ordered logit — review score (Ch 12.3)

Treating a 1-5 review score as continuous would imply the gap from 1→2 stars equals the gap from 4→5 — false in practically every Likert instrument. McElreath §12.3 prescribes a cumulative-link model with K-1 cutpoints:
...
Three variations to mix and match — pick the one that fits the role description.

> **Marketplace / e-commerce focus (Shopee, ShopBack, TikTok Shop)**
> Built a hierarchical Bayesian A/B-testing framework on 100k+ Olist marketplace orders using DuckDB SQL feature pipeline (CTEs, window functions, gap-and-island session reconstruction) + PyMC; quantified posterior probability of treatment dominance across 73 product categories, surfacing Simpson-paradox effects invisible to pooled t-tests.

> **Statistical depth focus (BCG X, Temus, Income Insurance)**
> Designed and fit three hierarchical Bayesian models (Binomial conversion, hurdle-LogNormal revenue, ordered-logit review score) on the same DAG-justified adjustment set; modelled 73 product-category clusters with non-centered partial-pooling priors per McElreath; compared against classical A/B baselines (z, Welch, Mann-Whitney, chi-square) and used WAIC / PSIS-LOO for model selection.

> **Pipeline / engineering focus (Micron, Infineon, Razer)**
> End-to-end production-style ETL on the Olist e-commerce dataset: bronze → silver → gold → analytics layered DuckDB pipeline with idempotent SQL transforms, automated quality-diagnostics table, and a PyMC modelling layer driven from a single feature module — all reproducible from raw CSVs in under 6 minutes on a laptop.
...
- McElreath, R. (2020). *Statistical Rethinking: A Bayesian Course with Examples in R and Stan* (2nd ed.). CRC Press.
  - Ch 6 (DAGs, the four elemental confounds, backdoor adjustment)
  - Ch 11 (binomial regression, logit link, relative vs. absolute effects)
  - Ch 12 (zero-inflated Poisson; ordered-categorical cumulative-logit)
  - Ch 13 (multilevel models, partial pooling, non-centered priors)

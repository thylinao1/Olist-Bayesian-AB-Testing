...
The spread in `delta_bar` across the three runs is +0.0004 logit (~0.00 pp on the probability scale at the ~89% baseline). The headline +1.5 pp on-time policy effect does not depend on the hyperprior choice; the posterior is dominated by the 97k-order likelihood rather than by the prior.

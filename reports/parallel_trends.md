...
Both cohorts move broadly in parallel across the panel. The vertical dashed line marks the cutover week. Both cohorts also drop together at the right edge, which is the delivery-grace censoring (orders too recent to have a delivery outcome) and is filtered out of the modelling panel via the 6-week grace window in `src/features.py`.
